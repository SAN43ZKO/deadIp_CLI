...
def build_markdown(
    target: str, resolved: dict, checks: dict, verdict: dict, user_ip_masked: str | None = None
) -> str:
    ...
    rows.append(("3", f"TLS {tls.get('port', 443)}", _fmt_bool_icon(tls_ok), det))
    ...
    rows.append(("4", f"MTR (TCP {tls.get('port', 443)})", _fmt_bool_icon(tr_ok), tr_det))
    ...
    if tr.get("russian_as"):
        ...
        if tr.get("russian_as"):
            lines.append("")
            lines.append(f"Российские AS в трассе: **{', '.join(tr['russian_as'])}**.")

    lines += ["", f"## Вердикт: {verdict['verdict'].upper()}", "", verdict["reason"], ""]
    return "\n".join(lines)
